...
- **Components (R3.3)**: The 16 new tools are `additive-preview` at contract `1.22.0`. Mutations use exact local node IDs; create/match uses opaque private `identityKey` (document-wide for components/sets, parent-scoped for instances). Resolve and validate every input before native writes; refuse remote references, consumers on owned deletion, and hidden nested-detach cascades. Read back fresh projections and report `unconfirmed` when Figma discards a write. The three older instance tools remain frozen, and R3.3 tools are absent from `apply_batch`. G1–G4 require an owner-confirmed disposable file; all four passed live on 2026-09-24, and the `r3.3` baseline is frozen. Content placed into an instance SLOT gets a new instance-scoped ID (`I<inst>;<slot>;<guid>`), so find it by name. The pre-insertion and reply IDs only resolve as aliases.
