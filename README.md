...
- `create_group` - Create one native group from explicitly named existing scene nodes under an explicit compatible parent. Members must be unique, on the same page, and non-overlapping in the hierarchy; the receipt reads their absolute bounds before and after instead of assuming geometry preservation
...
- `set_fill_style` - Attach one exact local paint-style ID to a node, or pass `styleId: null` to clear its attachment. It uses Figma's async attachment API, refuses remote/library IDs, and does not create, import, or edit a style resource; the receipt reads the attachment before and after
...
- `get_local_style` - Read one exact current-file local paint, text, effect, or grid style with its stored value, readability, private-identity status (never the identity value), and optional consumer count. It never treats a Figma library key or a name as a selector
- `create_or_match_local_style` - Create one owned local style or match exactly one same-kind style carrying an opaque private `identityKey`. Same-kind name collisions without that marker are refused rather than adopted; remote, import, publish, and key-based paths are absent
- `update_local_style` - Change one owned local style by exact local ID plus matching private identity. Each call changes exactly one native property — either the name or the kind-specific value — and reads it back; styles with variable bindings R3.2 cannot preserve are refused
- `get_node_style_attachment` - Read a node's direct plugin style ID for paint fill/stroke, text, effect, or grid, distinguishing local, remote, wrong-kind, unknown, mixed, unreadable, and unmeasured references without joining REST style keys
...
- `delete_variable_collection` - Permanently remove one existing **empty** local variable collection only with literal `confirm: true`. It reads `variableIds` before calling Figma and refuses a non-empty collection as `collection_not_empty`, reporting its exact pre-call variable count and IDs rather than silently deleting those variables. After `remove()`, it probes both the exact lookup and local collection inventory; when neither can confirm absence in-frame, it returns `removal_unconfirmed` with `verificationDeferred` and requires a later read
