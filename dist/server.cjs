...
// src/talk_to_figma_mcp/runtime-compatibility.mjs
function comparePluginRuntimeMetadata(expected, plugin, checkedAt = (/* @__PURE__ */ new Date()).toISOString()) {
...
function runtimeDiagnosticSuffix() {
...
function comparePluginRuntime(plugin) {
  return comparePluginRuntimeMetadata(RUNTIME_METADATA, plugin);
}
async function refreshRuntimeCompatibility() {
  try {
...
server.tool(
  ...
  {
    summary: import_zod.z.boolean().optional().default(true).describe(
    ...
    limit: import_zod.z.number().int().min(1).max(500).optional().default(100).describe("Maximum current-page children returned (default: 100)"),
    offset: import_zod.z.number().int().min(0).optional().default(0).describe("Offset into the current page's children"),
    familyLimit: import_zod.z.number().int().min(1).max(500).optional().default(100).describe("Maximum child name families returned in summary mode")
  },
...
server.tool(
  ...
  {
    includeChildCount: import_zod.z.boolean().optional().default(false).describe(
...
server.tool(
  ...
  {
    pageId: import_zod.z.string().describe("The PAGE node ID returned by get_pages")
  },
  async ({ pageId }) => {
    try {
...
server.tool(
  ...
  {
    name: import_zod.z.string().describe("Name for the new page"),
    onDuplicate: import_zod.z.enum(["error", "allow"]).optional().describe(
    ...
    index: import_zod.z.number().int().optional().describe(
...
server.tool(
  ...
  "[Multi-node scoped] Apply many node mutations in one call, against node IDs that already exist. Every target is resolved in one pass before anything is written, and the resolved scope is reported either way. Creates are not accepted in v1. Returns a typed per-operation receipt correlated by your own `id`, and an `outcome` that cannot report success when nothing succeeded. Refusals for a duplicate `id` or a disallowed `op` are thrown, not returned. Operations are NOT atomic: a failed operation on a multi-field mutation may have partially applied, and says so. Runs in chunks of 5 with progress updates; `timeBudgetMs` is the total ceiling and is enforced regardless",
  {
    operations: import_zod.z.array(
      import_zod.z.object({
        id: import_zod.z.string().min(1).describe("Your own identifier for this operation. Receipts correlate by it, never by array position, so it must be unique within the batch"),
        ...
        nodeId: import_zod.z.string().min(1).describe("The existing node to mutate. Lifted out of `params` deliberately: this is the field prevalidation resolves, and it wins over any nodeId inside `params`"),
        params: import_zod.z.record(import_zod.z.any()).optional().describe("The parameters that operation takes, minus nodeId. \u26A0\uFE0F These go straight to the plugin handler, so for two operations they are NOT the standalone tool's shape: set_fill_color and set_stroke_color take {color:{r,g,b,a}} here (plus weight for the stroke), where the standalone tools take flat r,g,b,a. Everything else matches its tool. This object is not schema-validated, so a wrong shape fails plugin-side and comes back as a failed receipt entry rather than a schema error")
      })
    ).min(1).max(200).describe("The operations to apply, in order"),
    onError: import_zod.z.enum(["stop", "continue"]).optional().describe('"stop" (default) refuses the whole batch if any target is unresolvable, and halts after the first failure. "continue" skips unresolvable targets and runs the rest'),
    prevalidateOnly: import_zod.z.boolean().optional().describe("true runs the resolve pass and returns the report without writing anything \u2014 a dry run against the live file. Default false"),
    timeBudgetMs: import_zod.z.number().int().min(1e3).max(24e4).optional().describe("Total wall clock for the whole batch, not per operation (default 60000). On exhaustion the remaining operations are skipped and `complete` is false"),
    maxResultBytes: import_zod.z.number().int().min(0).optional().describe("Truncate each operation's result above this many UTF-8 bytes (default 2000). The true size is still reported as `resultBytes`"),
    chunkPauseMs: import_zod.z.number().int().min(0).max(5e3).optional().describe("Milliseconds to yield between chunks of 5 operations (default 0). Raise it only if Figma's UI needs to breathe on a heavy batch \u2014 the pause is skipped once timeBudgetMs is spent, so it can never push a run past its own ceiling")
  },
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).min(1).max(100).describe("1-100 existing scene-node IDs to group. Duplicates and ancestor/descendant pairs are refused before grouping."),
    parentId: import_zod.z.string().describe("Explicit PAGE, FRAME, GROUP, SECTION, or other child-accepting parent in the same page as every member."),
    index: import_zod.z.number().int().nonnegative().optional().describe("Optional insertion index in the explicit parent. Omit to let Figma place the new group after its existing children.")
  },
  async (args2) => {
    try {
...
server.tool(
  ...
  "Create a new text element in Figma, with the same typography parameters set_text_style writes onto an existing node. \u26D4 Validate-all-then-create: every parameter is checked, the parent resolved and the font loaded BEFORE the node is created, so a refusal leaves no orphan node on the page. \u26D4 An unloadable font is REFUSED, never substituted \u2014 this tool will not create a node in a face nobody asked for, so preflight with check_fonts and expect an error rather than a fallback. fontWeight reaches Inter's styles only and cannot be combined with fontFamily/fontStyle; supply the pair to name any installed face exactly.",
  {
    ...
    fontWeight: import_zod.z.number().optional().describe(
    ...
    lineHeight: import_zod.z.object({
      value: import_zod.z.number().nonnegative().optional().describe("Omit when unit is AUTO; supplying it there is refused rather than discarded."),
    ...
    letterSpacing: import_zod.z.object({
      value: import_zod.z.number().describe("May be negative; tracking-in is legitimate."),
      unit: import_zod.z.enum(["PIXELS", "PERCENT"])
    }).optional().describe("Letter spacing as {value, unit}. AUTO is not a letter-spacing unit."),
    textCase: import_zod.z.enum(["ORIGINAL", "UPPER", "LOWER", "TITLE", "SMALL_CAPS", "SMALL_CAPS_FORCED"]).optional().describe("Letter case transform applied for display; the underlying characters are unchanged."),
    textDecoration: import_zod.z.enum(["NONE", "UNDERLINE", "STRIKETHROUGH"]).optional().describe("Text decoration."),
    textAlignHorizontal: import_zod.z.enum(["LEFT", "CENTER", "RIGHT", "JUSTIFIED"]).optional().describe("Horizontal alignment within the text box."),
    textAlignVertical: import_zod.z.enum(["TOP", "CENTER", "BOTTOM"]).optional().describe("Vertical alignment within the text box."),
    paragraphSpacing: import_zod.z.number().nonnegative().optional().describe("Space between paragraphs, in pixels."),
    paragraphIndent: import_zod.z.number().nonnegative().optional().describe("First-line indent, in pixels."),
    textAutoResize: import_zod.z.enum(["NONE", "HEIGHT", "WIDTH_AND_HEIGHT", "TRUNCATE"]).optional().describe(
...
server.tool(
  ...
  "[Node scoped] Export a node as an image from Figma. Always returns a JSON receipt identifying the export plus a preflight cost estimate. PNG/JPG exports above the fork's 16 MP safety ceiling are refused unless allowLargeExport is explicitly true. Pass filePath to write the bytes to disk and keep base64 out of the transcript entirely. Export failures retain Figma's rejection text and thrown value type, with the error prefix once.",
  {
    ...
    allowLargeExport: import_zod.z.boolean().optional().default(false).describe(
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the TEXT node to restyle"),
    fontFamily: import_zod.z.string().optional().describe(
    ...
    fontSize: import_zod.z.number().min(1).max(65535).optional().describe("Font size in pixels."),
    lineHeight: import_zod.z.object({
      value: import_zod.z.number().nonnegative().optional().describe("Omit when unit is AUTO; supplying it there is refused rather than discarded."),
      unit: import_zod.z.enum(["PIXELS", "PERCENT", "AUTO"])
    }).optional().describe(
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the TEXT node to mutate."),
    start: import_zod.z.number().int().nonnegative().describe("Inclusive character offset."),
    end: import_zod.z.number().int().nonnegative().describe("Exclusive character offset; must be greater than start and at most the text length."),
    fontFamily: import_zod.z.string().min(1).describe("Exact, case-sensitive Figma font family to load and apply."),
    fontStyle: import_zod.z.string().min(1).describe("Exact, case-sensitive Figma font style to load and apply.")
  },
  async (args2) => {
    try {
...
server.tool(
  ...
  "[Document-wide by default, or scoped to chosen pages] Get local components. Summary mode is the default and returns counts plus bounded name families; set summary=false for a paginated component list. COST WARNING: every page must be fully loaded before it can be scanned, so runtime tracks page weight, not component count \u2014 on a large document this can exceed two minutes. Pass `pages` to scan only what you need, and/or `timeBudgetMs` to bound it. The reply always declares its own coverage via `complete`, `pagesScanned` and `pagesSkipped`, so a scoped or truncated scan is never mistakable for a document total. INTERPRETING THE COUNT: summary mode also returns `authoringSessions`, clustering components by the leading segment of their node id, which every node created in one authoring session shares. A bulk-pasted vendor kit therefore collapses into one or two sessions (usually low-numbered, since they were pasted early) while hand-authored work spreads across others \u2014 so a raw `count` often describes a purchased library rather than the designer's work. Treat the clusters as evidence for that judgement, not as a verdict: the tool deliberately does not label any session 'a kit'.",
  {
    ...
    sessionLimit: import_zod.z.number().int().min(1).max(100).optional().default(20).describe("Maximum authoring sessions returned in summary mode")
  },
  async ({
    summary,
    limit,
...
server.tool(
  ...
  {
    types: import_zod.z.array(import_zod.z.enum(["COLOR", "FLOAT", "STRING", "BOOLEAN"])).min(1).optional().describe("Optional variable types to include; defaults to all four types")
  },
  async ({ types }) => {
    try {
...
server.tool(
  ...
  {
    collectionId: import_zod.z.string().min(1).describe("ID of the existing local variable collection to change"),
    name: import_zod.z.string().min(1).describe("Name of the mode to add; this is a real document write, not a probe")
  },
  async (args2) => {
    try {
...
server.tool(
  ...
  {
    variableId: import_zod.z.string().min(1).describe("ID of the existing local variable to change"),
    modeId: import_zod.z.string().min(1).describe("ID of an existing mode in that variable's local collection"),
    value: import_zod.z.union([
      import_zod.z.string().describe("Raw STRING value"),
      import_zod.z.number().finite().describe("Raw finite FLOAT value"),
      import_zod.z.boolean().describe("Raw BOOLEAN value"),
      import_zod.z.object({
        ...
        a: import_zod.z.number().min(0).max(1).optional().describe("Alpha channel, 0\u20131; defaults to 1")
      }).strict().describe("Raw COLOR value as RGBA floats; no hex-string form")
    ]).optional().describe("Raw value; supply this XOR aliasOf"),
    aliasOf: import_zod.z.string().min(1).optional().describe("Existing local variable ID to alias; supply this XOR value")
  },
  async (args2) => {
...
server.tool(
  ...
  {
    variableId: import_zod.z.string().min(1).describe("ID of the existing local variable to permanently remove"),
...
server.tool(
  ...
  {
    collectionId: import_zod.z.string().min(1).describe("ID of the existing empty local variable collection to permanently remove"),
    confirm: import_zod.z.literal(true).describe("Required explicit destructive confirmation; must be true, not merely truthy")
  },
  async (args2) => {
    try {
...
server.tool(
  ...
  {
    name: import_zod.z.string().min(1).describe("Name for the collection; this is a real document write"),
    id: import_zod.z.string().min(1).optional().describe("Optional exact local variable collection ID; when supplied it is the first identity layer and a miss refuses rather than creating"),
    identityKey: import_zod.z.string().min(1).optional().describe("Optional opaque caller-owned string for idempotent identity; stored privately on a newly created or matching untagged collection and never interpreted or returned")
  },
  async (args2) => {
    try {
      const result = await sendCommandToFigma("create_variable_collection", args2);
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  "[Exact local variable collection + exact mode, caller-requested write] Rename one mode of one existing local variable collection. Nothing a variable resolves through changes: values, the default mode, and every other mode are untouched. A rename to the name the mode ALREADY has is REFUSED as mode_name_unchanged rather than reported as applied, because a no-op rename and a rename that silently failed produce identical bytes. \u26A0\uFE0F MEASURED: Figma ACCEPTS a rename that duplicates another mode's name in the same collection, so this tool does not invent a refusal Figma does not have \u2014 instead the receipt reports nameCollidesWithModeIds, read before the call, naming every other mode already using the requested name. It is a reading, never a refusal. Remote collections and a modeId that does not belong to the named collection get typed refusals. After renameMode() the handler probes independent in-frame signals and names the one that observed the new name; when none can, it reports outcome rename_unconfirmed with verificationDeferred instead of claiming the rename.",
  {
    ...
    name: import_zod.z.string().min(1).describe("New name for the mode; must differ from its current name")
  },
  async (args2) => {
    try {
      const result = await sendCommandToFigma("rename_variable_mode", args2);
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  {
    variableId: import_zod.z.string().min(1).describe("ID of the existing local variable to change"),
    name: import_zod.z.string().min(1).optional().describe("New variable name; omit to leave it unchanged"),
    description: import_zod.z.string().optional().describe("New description; the empty string clears it. Omit to leave it unchanged"),
    scopes: import_zod.z.array(
      import_zod.z.enum([
        "ALL_SCOPES",
        "ALL_FILLS",
        "FRAME_FILL",
    ...
    ).min(1).optional().describe("Complete replacement list of Figma variable scopes; this REPLACES the current scopes rather than adding to them. Omit to leave them unchanged")
  },
  async (args2) => {
    const supplied = ["name", "description", "scopes"].filter(
      (field) => Object.prototype.hasOwnProperty.call(args2, field) && args2[field] !== void 0
    );
    ...
    try {
      const result = await sendCommandToFigma("set_variable_metadata", args2);
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().min(1).describe("ID of the existing node to bind the variable on"),
    field: import_zod.z.string().min(1).describe("Bindable plain field name, e.g. width, height, characters, fontSize, itemSpacing, paddingLeft, topLeftRadius, visible, opacity"),
    variableId: import_zod.z.string().min(1).describe("ID of the existing local variable to bind; its resolved type must suit the field")
  },
  async (args2) => {
    try {
      const result = await sendCommandToFigma("bind_variable_to_node", args2);
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().min(1).describe("ID of the existing node that owns the paint"),
    paintTarget: import_zod.z.enum(["fills", "strokes"]).describe("Which paint list on the node holds the paint to bind"),
    paintIndex: import_zod.z.number().int().min(0).describe("Zero-based index of the paint within that list; it must already exist"),
    variableId: import_zod.z.string().min(1).describe("ID of the existing local COLOR variable to bind to the paint's colour")
  },
  async (args2) => {
    try {
      const result = await sendCommandToFigma("bind_variable_to_paint", args2);
      return {
        content: [
          {
            type: "text",
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().optional().describe("Optional root node ID; omit to scan the current page"),
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node to constrain (the CHILD, not the parent frame)"),
    horizontal: import_zod.z.enum(["MIN", "CENTER", "MAX", "STRETCH", "SCALE"]).optional().describe(
    ...
    vertical: import_zod.z.enum(["MIN", "CENTER", "MAX", "STRETCH", "SCALE"]).optional().describe(
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node whose fill-style attachment changes."),
    styleId: import_zod.z.string().min(1).nullable().describe("Exact ID returned by get_styles.colors for a local paint style, or null to clear the attachment. Remote/library IDs are refused.")
  },
  async (args2) => {
    try {
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().min(1).describe("Exact current-file node ID"),
    includeInstances: import_zod.z.boolean().optional().describe("Read bounded instance consumers for a local component or set")
  },
  async (args2) => {
...
server.tool(
  ...
  {
    parentId: import_zod.z.string().min(1),
...
server.tool(
  ...
  {
    parentId: import_zod.z.string().min(1),
    componentId: import_zod.z.string().min(1).optional(),
    componentSetId: import_zod.z.string().min(1).optional(),
    variantProperties: import_zod.z.record(import_zod.z.string()).optional(),
    identityKey: import_zod.z.string().min(1).max(2048).optional()
  },
  async (args2) => {
...
server.tool(
  ...
  "Remove one exact owned local standalone component or whole component set only after a complete consumer read proves that no instances use it. Never cascades.",
  {
    nodeId: import_zod.z.string().min(1),
    identityKey: import_zod.z.string().min(1).max(2048),
    confirm: import_zod.z.literal(true)
  },
  async (args2) => {
...
server.tool(
  ...
  {
    componentIds: import_zod.z.array(import_zod.z.string().min(1)).min(2).max(100),
    parentId: import_zod.z.string().min(1),
    identityKey: import_zod.z.string().min(1).max(2048)
  },
  async (args2) => {
...
server.tool(
  ...
  "Add a supported property to an exact local standalone component or set. INSTANCE_SWAP values and preferred values are exact local node IDs, resolved before deriving Figma keys.",
  {
    ...
    defaultValue: import_zod.z.union([import_zod.z.string(), import_zod.z.boolean()]),
    preferredValueIds: import_zod.z.array(import_zod.z.string().min(1)).max(50).optional()
  },
  async (args2) => {
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().min(1),
    propertyKey: import_zod.z.string().min(1),
    name: import_zod.z.string().min(1).optional(),
    defaultValue: import_zod.z.union([import_zod.z.string(), import_zod.z.boolean()]).optional(),
    preferredValueIds: import_zod.z.array(import_zod.z.string().min(1)).max(50).optional(),
    description: import_zod.z.string().optional(),
    slotSettings: import_zod.z.object({
      stretchChildOnInsert: import_zod.z.boolean().optional(),
      displayEmptyByDefault: import_zod.z.boolean().optional(),
      minChildren: import_zod.z.number().int().min(0).nullable().optional(),
      maxChildren: import_zod.z.number().int().min(0).nullable().optional(),
      allowPreferredValuesOnly: import_zod.z.boolean().optional()
    }).strict().optional()
  },
  async (args2) => {
...
server.tool(
  ...
  "Bind or clear a TEXT, BOOLEAN, or INSTANCE_SWAP property reference on a child of an exact local component or set. A null propertyKey clears the selected field.",
  {
    nodeId: import_zod.z.string().min(1),
    componentId: import_zod.z.string().min(1).optional(),
    field: import_zod.z.enum(["characters", "visible", "mainComponent"]),
    propertyKey: import_zod.z.string().min(1).nullable()
  },
  async (args2) => {
...
server.tool(
  ...
  "Set one or more existing component properties on an exact instance after validating every key and value. SLOT properties are refused; variant combinations must resolve to one local member.",
  {
    ...
    properties: import_zod.z.record(import_zod.z.union([import_zod.z.string(), import_zod.z.boolean()]))
  },
  async (args2) => {
...
server.tool(
  ...
  {
    instanceId: import_zod.z.string().min(1),
    componentId: import_zod.z.string().min(1)
  },
  async (args2) => {
...
server.tool(
  ...
  { slotId: import_zod.z.string().min(1) },
  async (args2) => {
...
server.tool(
  ...
  {
    nodeId: import_zod.z.string().describe("The ID of the node whose effects are being replaced"),
    effects: import_zod.z.array(
      import_zod.z.object({
        ...
        color: import_zod.z.object({
          r: import_zod.z.number().finite().min(0).max(1).describe("Red channel, 0-1"),
          g: import_zod.z.number().finite().min(0).max(1).describe("Green channel, 0-1"),
          b: import_zod.z.number().finite().min(0).max(1).describe("Blue channel, 0-1"),
          a: import_zod.z.number().finite().min(0).max(1).optional().describe("Alpha, 0-1. Defaults to 1; a shadow has no second opacity spelling")
        }).optional().describe("Required by DROP_SHADOW and INNER_SHADOW; refused on blur effects"),
        offset: import_zod.z.object({
          x: import_zod.z.number().finite().describe("Horizontal offset in pixels"),
          y: import_zod.z.number().finite().describe("Vertical offset in pixels")
        }).optional().describe("Required by DROP_SHADOW and INNER_SHADOW; refused on blur effects"),
        radius: import_zod.z.number().finite().min(0).optional().describe("Required blur or shadow radius, greater than or equal to 0"),
        spread: import_zod.z.number().finite().optional().describe("Optional shadow spread. Its sign is intentionally not constrained"),
        visible: import_zod.z.boolean().optional().describe("Whether this effect is drawn"),
        blendMode: import_zod.z.enum([
          "NORMAL",
          "DARKEN",
          "MULTIPLY",
        ...
        showShadowBehindNode: import_zod.z.boolean().optional().describe("Optional DROP_SHADOW-only setting; refused on every other effect type")
      }).passthrough()
...
server.tool(
  ...
  {
    nodeIds: import_zod.z.array(import_zod.z.string()).min(1).describe("Array of root node IDs whose subtrees should be searched")
  },
  async ({ nodeIds }) => {
    try {
