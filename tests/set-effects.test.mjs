...
test("the R3-A capability preflight keeps all three 1.10.0 version fields together", async () => {
  const release = JSON.parse(
    await readFile(path.join(root, "runtime/release.json"), "utf8"),
  );
  assert.equal(release.publicContractVersion, "1.10.0");
  assert.equal(release.serverSchemaVersion, "1.10.0");
  assert.equal(release.pluginApiVersion, "1.10.0");
});
