...
test("the public contract keeps two stable node writes while R3-A schema 1.14.0 holds", async () => {
  ...
  assert.equal(built.contract.publicContractVersion, "1.14.0");
  assert.equal(built.contract.serverSchemaVersion, "1.14.0");
  assert.equal(built.release.pluginApiVersion, "1.14.0");
});
