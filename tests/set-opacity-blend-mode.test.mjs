...
test("the public contract keeps two stable node writes while R3.2.1 schema 1.21.0 holds", async () => {
  ...
  assert.equal(built.contract.publicContractVersion, "1.23.0");
  assert.equal(built.contract.serverSchemaVersion, "1.23.0");
  assert.equal(built.release.pluginApiVersion, "1.23.0");
});
