...
test("the public contract declares two stable node writes while schema 1.9.0 holds", async () => {
  ...
  assert.equal(built.contract.publicContractVersion, "1.9.0");
  assert.equal(built.contract.serverSchemaVersion, "1.9.0");
  assert.equal(built.release.pluginApiVersion, "1.9.0");
});
